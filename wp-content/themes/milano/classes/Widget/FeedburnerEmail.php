...
class Widget_FeedburnerEmail extends Widget_Default
{
	...
	function widget( $args, $instance ) {
        ...
        <form   action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $name ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
			<fieldset>
				...
				<input type="text"  name="email" id="email<?php global $wid; echo $wid ?>" placeholder="<?php _e('Your E-Mail', 'milano'); ?>" />
	...
	function form( $instance ) {
		...
		// Defaults
		$defaults = array( 'title' => __( 'Sign up for our Newsletter', 'milano' ),'description' => __('Sign up for our mailing list to stay updated on the latest themes from the Themoholics!', 'milano'), 'feedname' => 'themoholics', 'btntext'=> 'Submit');
		...
		<div>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'description' ); ?>"><?php _e( 'Description:', 'milano' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'feedname' ); ?>"><?php _e( 'Write only feedburner name:', 'milano' ); ?></label>
