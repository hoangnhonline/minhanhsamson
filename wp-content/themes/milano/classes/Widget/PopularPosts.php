...
class Widget_PopularPosts extends Widget_Default implements Widget_Interface_Cache
{
					...
					<?php if($current_post_id != get_the_ID() && $shown_post <= $instance[self::NUMBER]):?>
						<li>
							<a href="<?php echo get_permalink(get_the_ID()); ?>" title="<?php the_title(); ?>" class="imgborder">
								...
								{
									echo theme_post_thumbnail('recent_posts');
								}
								else {echo '<span class="placeholder"><span></span></span>';}
								?>        
							</a>
							<div class="recent_txt">
								<a class="title" href="<?php echo get_permalink(get_the_ID()); ?>" title="<?php the_title(); ?>" ><?php the_title(); ?></a>
								<?php 
									echo "<p>";
									if (comments_open()) { comments_popup_link(__('0 - Comments', 'milano'), __('1 - Comment', 'milano'), __('% - Comments', 'milano'), 'comments');} 
									echo "</p>"
								?>
							</div>
						</li>
	...
	function form($instance)
	{
		...
		<div>
			<p>
				<label for="<?php echo $this->get_field_id(self::TITLE); ?>"><?php _e('Title:', 'milano'); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id(self::NUMBER); ?>"><?php _e('Number of posts to show:', 'milano'); ?></label>
	...
	private function getDefaultFieldValues()
	{
		$default_values = array(
					self::TITLE		=> __('Popular posts', 'milano'),
					self::NUMBER	=> 5,
					);
