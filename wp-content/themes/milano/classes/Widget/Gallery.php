...
class Widget_Gallery extends Widget_Default implements Widget_Interface_Cache
{
		...
		if ($wport->have_posts()) : ?>
			...
			<?php  while($wport->have_posts()) : $wport->the_post();?>
				<li class="<?php if( ($wport->current_post % 2 ) == 0  ) { echo("first");} ?>" >
							...
							<a href="<?php the_permalink() ?>" title="<?php echo the_title(); ?>" class="thumb">
	...
	function form( $instance ) {
		...
		// Defaults
		$gallery_terms = '';
		$defaults = array( 'title' => __( 'From gallery', 'milano' ), 'number' => '4');
		...
		<div>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'category' ); ?>" >
					<?php _e( 'Category of gallery:', 'milano' ); ?>
				</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of items to show:', 'milano' ); ?></label>
