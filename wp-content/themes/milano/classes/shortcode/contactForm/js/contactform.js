...
FB.ContactForm = function() {
	...
	'action' : 'th_contactform_shortcode',
	...
	var thNewCount = 0; // increment for new fields
	var maxNewFields = 75;  // Limits number of new fields available
	var optionsCache = {};
	...
	function addField () {
		try {
			jQuery('#fb-reply').hide();
			thNewCount++;
				jQuery('#fb-extra-fields').before('<div id="fb-new-field' + thNewCount + '" fieldid="' + thNewCount + '" class="fb-new-fields"><div class="fb-fields"><div id="' + thNewCount + '" class="fb-remove"></div><label fieldid="' + thNewCount + '" for="fb-field' + thNewCount + '"><span class="label-text">New field</span> </label><input type="text" id="fb-field' + thNewCount + '" disabled="disabled" /></div></div>');
				// Add to form object
				fbForm.fields[thNewCount] = {
					'label' : 'New Field',
					'type' : 'text',
					'reply' : false,
					'required' : false,
				...
				jQuery('#fb-field-id').val(thNewCount);
	...
	function buildPreview () {
		try {
			...
			if(fbForm.button_text) { jQuery('#fb-sendbutton').val(fbForm.button_text);}
	...
	function loadFieldEditor (id) {
		try {
			...
			{
				jQuery('#fb-new-reply').prop("checked", fbForm.fields[id].reply);
				jQuery('#fb-reply').show();
			}
	...
	function sendShortcodeToEditor () {
		try {
			...
			jQuery.post(ajaxurl, fbForm, function(response) {
				...
				var regexp = new RegExp("\\[th-contact-form\\b.*?\\/?\\](?:[\\s\\S]+?\\[\\/th-contact-form\\])?");
	...
	function updateSendButtonText() {
		try {
			var new_send_button_text = jQuery('#fb-sendbutton').val();
			fbForm.button_text = new_send_button_text;
		}
		catch (e) {
			if (debug) {
				console.log("updateSendButtonText(): " + e);
			}
		}
	}
	...
	function updateReply() {
		try {
			var thisId = jQuery('#fb-field-id').val();
			fbForm.fields[thisId].reply = jQuery('#fb-new-reply').is(':checked');
		} catch(e) {
			if (debug) {
				console.log("updateReply(): " + e);
			}
		}
	}
	...
	function updateType(thisType, thisLabelText, thisRequired) {
		try {
			...
			switch (thisType) {
				...
				case "email":
					removeOptions();
					jQuery('#fb-new-reply').prop("checked", false);
					jQuery('#fb-reply').show();
	...
	return {
		...
		init: function () {
			...
			jQuery('#fb-sendbutton').change(function(){
			...
			jQuery('#fb-new-reply').click(function () {
				updateReply();
			});
