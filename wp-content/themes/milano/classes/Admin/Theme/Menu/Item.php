...
abstract class Admin_Theme_Menu_Item
{
	 ...
	 */
	const NONCE_SAVE_ACTION = 'lf-save-action_nonce';
	
	/**
	 * Reset action name for wp_nonce_field && check_admin_referer 
	 */
	const NONCE_RESET_ACTION = 'lf-reset-action_nonce';
	/**
	...
	public function renderPage()
	{
		...
		if($this->getMenuSlug() != SHORTNAME.'_dummy')
		{
			...
			$html .= '<!--<div class="color_separator clear">
			...
			$html .= '<input name="save_options" type="submit" value="Save Changes" class="ml_save"  />'.
				$save_nonce	
				.'</form>
			<form method="post" id="ml_reset"  action="">'.
				$reset_nonce
				.'<input type="hidden" name="reset_options" id="reset_options" /><input name="reset_options" type="submit" value="Reset" class="ml_reset" />
