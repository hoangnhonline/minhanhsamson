...
// */
//if ( class_exists( 'WP_Importer' ) ) {
class Import_Importer extends WP_Importer {
	...
	/**
	 * Clear terms, terms_relationship, terms_taxonomy tables
	 * and reset it AUTO_INCREMENT value
	 * @global type $wpdb
	 */
	function trancate_tables()
	{
		global $wpdb;
	...
	function import_end() {
		...
		echo '<div class="highlight"><p>' . __( 'All done.', 'wordpress-importer' ) . ' <a href="' . admin_url() . '">' . __( 'Have fun!', 'wordpress-importer' ) . '</a>' . '</p></div>';
...
<?php
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'milano' ), esc_html($this->authors[$old_login]['author_display_name']) );
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			$wpdb->insert( $wpdb->terms, array('term_id'=>$cat['term_id'], 'name'=>$cat['cat_name'], 'slug'=>$cat['category_nicename'], 'term_group'=>0) );
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			$wpdb->insert( $wpdb->terms, array('term_id'=>$tag['term_id'], 'name'=>$tag['tag_name'], 'slug'=>$tag['tag_slug'], 'term_group'=>0) );
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			{
				$wpdb->insert( $wpdb->terms, array('term_id'=>$term['term_id'], 'name'=>$term['term_name'], 'slug'=>$term['slug'], 'term_group'=>0) );
			}
			if(!term_exists($term['term_id'], $term['term_taxonomy']))
			{
				$wpdb->insert( $wpdb->term_taxonomy, array('term_id'=>$term['term_id'], 'taxonomy'=>$term['term_taxonomy']) );
			}
...
//	load_plugin_textdomain( 'milano', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
//
//	/**
...
//	register_importer( 'wordpress', 'WordPress', __('Import <strong>posts, pages, comments, custom fields, categories, and tags</strong> from a WordPress export file.', 'milano'), array( $GLOBALS['wp_import'], 'dispatch' ) );
//}
