...
class Envato_Theme_Backup
{
	...
	public static function get_instance()
	{
		...
		if (empty(self::$instance))
			self::$instance = new Envato_Theme_Backup();
	...
	public function pcl_zip()
	{
		...
		global $theme_envato_backup_exclude_string;

		$theme_envato_backup_exclude_string = $this->exclude_string('regex');
		...
		/* Zip up everything */
		if (!$archive->add($this->root(), PCLZIP_OPT_REMOVE_PATH, $this->root(), PCLZIP_CB_PRE_ADD, 'theme_envato_backup_pclzip_callback'))
			$this->warning($this->archive_method, $archive->errorInfo(true));

		unset($GLOBALS['theme_envato_backup_exclude_string']);
	...
	public function check_archive()
	{
		...
		if (!file_exists($this->archive_filepath()))
			$this->error($this->archive_method(), __('The backup file was not created', 'milano'));
	...
	public function files()
	{
		...
		if (!empty($this->unreadable_files))
			$this->warning($this->archive_method(), __('The following files are unreadable and could not be backed up: ', 'milano') . implode(', ', $this->unreadable_files));
 ...
 * @return    bool
 */
function theme_envato_backup_pclzip_callback($event, &$file)
{
	...
	elseif ($theme_envato_backup_exclude_string && preg_match('(' . $theme_envato_backup_exclude_string . ')', $file['stored_filename']))
		return false;
