...
class Custom_MetaBox_Taxonomy {
	...
	public function __construct ( $meta_box ) {
		...
		if (isset($meta_box['use_with_theme']))
			if ($meta_box['use_with_theme'] === true)
				{
					$this->SelfPath = get_stylesheet_directory_uri() . '/inc/Tax-meta-class';
				}
	...
	public function load_scripts_styles() {
		...
		if (in_array($taxnow,$this->_meta_box['pages'])){
			...
			wp_enqueue_style( 'th-admin-css', $plugin_path . '/css/admin.css' );
			// Enqueue Meta Box Scripts
			wp_enqueue_script( 'tax-meta-clss', $plugin_path . '/js/backend/tax-meta-clss.js', array( 'jquery' ), null, true );
	...
	public function insert_images() {
		...
		foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
			...
			$li 	.= "<a title='" . __( 'Delete this image', 'milano' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$term_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Delete', 'milano' ) . "' /></a>";
	...
	*/
	public function wp_ajax_delete_image() {
		...
		if ( $ok ){
			...
			echo json_encode(array('message' => __( 'Cannot delete file. Something\'s wrong.', 'milano')));
			die();
		}
	}
	...
	public function check_field_color() {
		...
		if ( $this->has_field( 'color' ) && $this->is_edit_page() ) {
			...
			add_action('init', array(&$this, 'color_init'));
		}

	}
	public function color_init() {
		wp_enqueue_style( 'farbtastic' );
		wp_enqueue_script( 'farbtastic' );
	}	
	...
	public function show_field_repeater( $field, $meta ) {
    	...
    	if (count($meta) > 0 && is_array($meta) ){
   			foreach ($meta as $me){
				...
				echo '" alt="'.__('Remove','milano').'" title="'.__('Remove','milano').'" id="remove-'.$field['id'].'"></div>';
				$c = $c + 1;
		...
		echo '" alt="'.__('Add','milano').'" title="'.__('Add', 'milano').'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
	...
	public function show_field_file( $field, $meta ) {
		...
		$this->show_field_begin( $field, $meta );
			...
			if ( ! empty( $meta ) ) {
				$nonce = wp_create_nonce( 'at_ajax_delete' );
				echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files', 'milano') . '</strong></div>';
				echo '<ol class="at-upload">';
				foreach ( $meta as $att ) {
					
					echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}|{$post->ID}|{$field['id']}|{$att}'>" . __( 'Delete', 'milano') . "</a>)</li>";
				}
				echo '</ol>';
			}
			...
			echo "<div class='at-file-upload-label'>";
				echo "<strong>" . __( 'Upload new files', 'milano') . "</strong>";
			echo "</div>";
			echo "<div class='new-files'>";
				echo "<div class='file-input'>";
					echo "<input type='file' name='{$field['id']}[]' />";
				echo "</div><!-- End .file-input -->";
				echo "<a class='at-add-file button' href='#'>" . __( 'Add more files', 'milano') . "</a>";
	...
	public function show_field_color( $field, $meta ) {
		...
		$this->show_field_begin( $field, $meta );
			...
			echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color', 'milano') . "'/>";
