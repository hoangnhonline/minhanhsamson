...
class Custom_Thumbnail_Multi
{
	...
	{
		add_meta_box("{$this->post_type}-{$this->id}",$this->label, array($this, 'thumbnail_meta_box'), $this->post_type, 'side', $this->priority);
	}
	...
	{
		wp_enqueue_script("featured-image-custom", get_template_directory_uri() . '/js/backend/multi-post-thumbnails-admin.js', array('jquery'));
	}
	...
	public static function get_the_post_thumbnail_src($post_type, $thumb_id, $post_id = NULL) {
		global $id;
		$post_id = (NULL === $post_id) ? $id : $post_id;
		$post_thumbnail_id = self::get_post_thumbnail_id($post_type, $thumb_id, $post_id);
	...
	private function post_thumbnail_html($thumbnail_id = NULL)
	{
		...
		$set_thumbnail_link = sprintf('<p class="hide-if-no-js"><a title="%1$s" href="%2$s" id="set-%3$s-%4$s-thumbnail" class="thickbox">%%s</a></p>', esc_attr("Set {$this->label}"), get_upload_iframe_src('image'), $this->post_type, $this->id);
		$content = sprintf($set_thumbnail_link, esc_html("Set {$this->label}"));
		...
		if ($thumbnail_id && get_post($thumbnail_id))
		{
			...
			if (!empty($thumbnail_html))
			{
				...
				$content .= sprintf('<p class="hide-if-no-js"><a href="#" id="remove-%1$s-%2$s-thumbnail" onclick="MultiPostThumbnailsRemoveThumbnail(\'%2$s\', \'%1$s\', \'%4$s\');return false;">%3$s</a></p>', $this->post_type, $this->id, esc_html("Remove {$this->label}"), $ajax_nonce);
			}
