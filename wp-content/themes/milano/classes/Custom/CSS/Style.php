...
class Custom_CSS_Style
{
	...
	private function getStyle()
	{
		...
		$isCustomizeStyle = $this->isCustomizeStyle();
		$isSessionPreview = $this->isSessionPreview();
			...
			if (is_string($macros) && strlen($macros))
			{
				if ($isCustomizeStyle)
				{
					if ($customazie_values && is_array($customazie_values))
					{
						...
						{
							$this->options_value[$macros] = $customazie_values[$macros];
						}
						else
						{
							$this->options_value[$macros] = $this->getMacrosValue($macros);
						}
					}
					else
					{
				...
				elseif ($isSessionPreview)
				{
					if (isset($_SESSION[$macros]))
					{
						$this->options_value[$macros] = $_SESSION[$macros];
					}
					else
					{
						$this->options_value[$macros] = $this->getMacrosValue($macros);
				...
				if ($this->isColorchooser($macros))
				{
					...
					$this->options_value[$macros_rbg] = $this->hex2rgb($this->options_value[$macros]);
				}
			}
		}
		 ...
		 * hack for GWF
		 */
		if (!get_option(SHORTNAME . '_gfontdisable')) // Google font disabled		
		{

			$ml_gfont_title = Admin_Theme_Element_Select_Gfont::compatibility(get_option(SHORTNAME . '_gfont_title'));
			$this->options_value['ml_gfont_title'] = sprintf("'%s'", $ml_gfont_title[0]['family']);
		}

		$ml_logo_gfont = Admin_Theme_Element_Select_Gfont::compatibility(get_option(SHORTNAME . '_logo_font'));
	$this->options_value['ml_logo_gfont'] = sprintf("'%s'", $ml_logo_gfont[0]['family']);

		/////////////////////////

		$content = preg_replace_callback('/\$([\w]+)/', array(&$this,'replaceCallback'), $this->getTemplateStyle());
	...
	private function replaceCallback (&$match) {
			if (!array_key_exists($match[1], $this->options_value))
			{
				return '';
			}
			return $this->options_value[$match[1]];
	}
	...
	function getMacrosValue($macros)
	{
		...
		$value = str_replace(array('http://', 'https://'), '//', get_option($macros));
